        ...
        if (recvPacket != nullptr) {
            ...
            while (auto chunk = recvPacket->peekAt(offset)->dupShared()) {  // for each chunk
                ...
                if (chunk->getClassName() == std::string("inet::SliceChunk")) {
                    auto newPacket = recvPacket->peekData<inet::SliceChunk>(inet::Chunk::PeekFlag::PF_ALLOW_ALL);
                    ...
                    if (encapsulatedChunk->getClassName() != std::string("CosimaApplicationChunk")) {
                        if (encapsulatedChunk->getClassName() == std::string("inet::SequenceChunk")) {
                            ...
                            for (int i=0; i < queue.size(); i++) {
                                ...
                                if (itemget->getClassName() == std::string("inet::SliceChunk")) {
                                    ...
                                    auto appChunk = appChunkInSliceChunk->peek<CosimaApplicationChunk>(inet::b(0), appChunkInSliceChunk->getChunkLength(), inet::Chunk::PeekFlag::PF_ALLOW_ALL);
                        ...
                        auto appChunk = encapsulatedChunk->peek<CosimaApplicationChunk>(inet::b(0), encapsulatedChunk->getChunkLength(), inet::Chunk::PeekFlag::PF_ALLOW_ALL);
                        if (appChunk->isTrafficMessage()) {
                            scheduler->log(nameStr + " received traffic message at time "+ simTime().str(), "info");
                            return;
                        } else {
                    ...
                    auto cosimaApplicationChunk = recvPacket->peekAt<CosimaApplicationChunk>(offset, length, inet::Chunk::PeekFlag::PF_ALLOW_ALL);
