...
from cosima_core.config import USED_STEP_SIZE, CONNECT_ATTR, SIMULATION_END, TIME_BASED
from cosima_core.messages.message_pb2 import CosimaMsgGroup
from cosima_core.simulators.core.omnetpp_connection import OmnetppConnection
...
class CommSimulator(mosaik_api.Simulator):
    ...
    def create_protobuf_msg(self, messages, sim_time):
        ...
        for message, size in messages:
            ...
            if message[
                "type"] is CosimaMsgGroup.CosimaMsg.MsgType.DISCONNECT or \
                    message[
                ...
                for idx, msg in enumerate(msgs):
                    if msg.type == CosimaMsgGroup.CosimaMsg.MsgType.INFO:
                        ...
                        if self._number_messages_received >= \
                            ...
                            self._number_messages_sent = 0
                            self._received_answer = True
                            self.send_waiting_msg(max_advance, SIMULATION_END)
                        else:
                        ...
                        if idx == len(msgs) - 1:
                            log(f'received max advance event at time {sim_time}'
                                ...
                                f' time: {msg.simTime}')
                            if TIME_BASED and msg.simTime == SIMULATION_END:
                                self.send_waiting_msg(max_advance, SIMULATION_END)
                            ...
                            if sim_time < msg.simTime:
                                ...
                                raise ValueError
                    elif msg.type == CosimaMsgGroup.CosimaMsg.MsgType.WAITING:
                        log('received info about continued waiting in OMNeT++')
                        if msg.simTime > sim_time:
                            log(f'set next step to {msg.simTime}')
                            next_step = msg.simTime
                            received_info_msg = True
    ...
    def send_message_to_omnetpp(self, messages, sim_time):
        ...
        """
        proto_message, message_count = self.create_protobuf_msg(messages,
                                                                sim_time)
    ...
    def check_msgs(messages, time):
        ...
        for message in messages:
            if type(message) is CosimaMsgGroup.CosimaMsg:
                if message.simTime > latest_message_time:
                    latest_message_time = message.simTime
                if TIME_BASED and message.simTime < time:
                    log(f'set message sim time to {time} instead of {message.simTime}')
    ...
    def step(self, time, inputs, max_advance):
        self._steps += 1
        print(f'step number {self._steps}')
        ...
        if time+1 >= SIMULATION_END and TIME_BASED:
            self.finalize()

        if len(messages_to_send) > 0:
            self._number_messages_sent += self.send_message_to_omnetpp(
                messages_to_send, time)
        ...
        if received_messages:
            ...
            if send_waiting_msg:
                self.send_waiting_msg(SIMULATION_END, SIMULATION_END)
    ...
    def finalize(self):
        ...
        print(f'simulation duration: {time.time() - self._start_time}')
