...
CALCULATING_TIMES = {"client0": 0, "client1": 0, "client2": 0}
...
def main():
    ...
    sim_config = {
        "Collector": {"python": "cosima_core.simulators.collector:Collector"},
        "Agent": {
    ...
    if CONTENT_LENGTH:
        message_content = "".join(
            random.choices(string.ascii_lowercase + string.digits, k=CONTENT_LENGTH)
        )
    else:
        message_content = None

    use_ict_simulator = (
        len(scenario_config.INFRASTRUCTURE_CHANGES) > 0
        or len(scenario_config.TRAFFIC_CONFIGURATION) > 0
        or len(scenario_config.ATTACK_CONFIGURATION) > 0
    ) and scenario_config.USE_COMMUNICATION_SIMULATION
    ...
    if len(AGENTS_WITH_HOUSEHOLDS) > 0:
        ...
        houses = household_sim.ResidentialLoads.create(
            1,
            sim_start=cfg.START,
    ...
    for idx, agent in enumerate(agents.values()):
        ...
        if agent.eid in AGENTS_WITH_HOUSEHOLDS:
            world.connect(
                houses.children[idx], agents[agent.eid], ("P_out", "P"), weak=True
            )
    ...
    if use_ict_simulator:
        ...
        world.connect(
            ict_controller, communication_simulator, f"ict_message", weak=True
        )
    ...
    for client_name, attr in client_attribute_mapping.items():
        ...
        world.connect(communication_simulator, agents[client_name], attr)
        world.connect(communication_simulator, collector, attr)
